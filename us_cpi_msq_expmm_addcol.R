# ================================================================================
# us_cpi_msq_expmm_addcol.R
#
# Rebuilds regime_tex/msq_expmm_120.tex (mismatched QoQ expectations, quarterly
...
#
# Only regime_tex/msq_expmm_120.tex is written.
# ================================================================================
...
## ---- data ----
cat("Building short and long mismatched (QoQ) panels...\n")
cpi <- fetch("CPIAUCSL")
...
sys_short <- na.omit(merge(Exp=mich_q_short, CPI=cpi_qoq)); Sm_short <- coredata(sys_short); colnames(Sm_short)<-c("Exp","CPI"); yq_short <- as.yearqtr(index(sys_short))
sys_long  <- na.omit(merge(Exp=mich_q_long,  CPI=cpi_qoq)); Sm_long  <- coredata(sys_long);  colnames(Sm_long) <-c("Exp","CPI"); yq_long  <- as.yearqtr(index(sys_long))
...
writeLines(body, "regime_tex/msq_expmm_120.tex")
cat("wrote regime_tex/msq_expmm_120.tex  (extended col =", yr_long, "; short cols from", yr_short, ")\n")
